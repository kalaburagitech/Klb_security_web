...
export const listPatrolLogs = query({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.siteId) {
            ...
            // We use the authorized site IDs as the primary filter source
            // If they don't exist (legacy/unrestricted), we fetch by org hierarchy
            let targetSiteIds = authorizedSiteIds;

            if (!targetSiteIds && args.organizationId) {
                // Legacy fallback: find all sites in org tree
